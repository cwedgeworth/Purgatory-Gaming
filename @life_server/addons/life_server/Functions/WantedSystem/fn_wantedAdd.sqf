...
switch(_type) do
{
    ...
    case "6": {_type = ["Reckless Driving",2500]};
    case "7": {_type = ["Attempted theft government vehicles",7500]};
    case "8": {_type = ["Theft of government vehicles",20000]};
    case "9": {_type = ["Flying in a no-fly zone",2500]};
    case "10": {_type = ["Driving illegal vehicles",10000]};
    case "11": {_type = ["Hit and run",5000]};
    case "12": {_type = ["Evading from police",7500]};
    case "13": {_type = ["Driving over another civilian",1500]};
    case "14": {_type = ["Illegal roadblocks",5000]};
    case "15": {_type = ["Resisting arrest",5000]};
    case "16": {_type = ["Not following orders",5000]};
    case "17": {_type = ["Harrasing citizens",2500]};
    case "18": {_type = ["Harassment to a police officer",8000]};
    case "19": {_type = ["Trespassing",5000]};
    case "20": {_type = ["Killing a police officer",25000]};
    case "21": {_type = ["Illegal discharge",5000]};
    case "22": {_type = ["Destruction of government property",15000]};
    case "23": {_type = ["Illegal Contraband",12000]};
    case "24": {_type = ["Possession of weapon without a license",5000]};
    case "25": {_type = ["Carrying a weapon inside city limits",2500]};
    case "26": {_type = ["Possession of a illegal weapon",10000]};
    case "27": {_type = ["Discharge of weapon within cities $10000",10000]};
...
if(_index != -1) then
{
	...
	[[_name,_uid,_crimes,(_type select 1) + _val],_uid] spawn TON_fnc_saveBounties; //wanted persistent
}
    else
{
    life_wanted_list pushBack [_name,_uid,[(_type select 0)],(_type select 1)];
	[[_name,_uid,[(_type select 0)],(_type select 1)],_uid] spawn TON_fnc_saveBounties; //wanted persistent
};